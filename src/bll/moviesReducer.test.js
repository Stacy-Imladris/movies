...
test('correct total movies count should be set', () => {
    const endState = moviesReducer(moviesStartState, moviesActions.setTotalMoviesCount(270))
...
test('correct error value should be set to the state', () => {
    const endState = moviesReducer(moviesStartState, moviesActions.setError(true))

    expect(endState.error).toBeTruthy()
})

test('correct title for search should be set', () => {
    const endState = moviesReducer(moviesStartState, moviesActions.setTitleForSearch('Harry'))

    expect(endState.titleForSearch).toBe('Harry')
})